...
impl From<SqlValue> for SltSqlValue {
    fn from(value: SqlValue) -> Self {
        match value {
            Bool(value) => SltSqlValue::Bool(value),
            I32(value) => SltSqlValue::Int(value),
            I64(value) => SltSqlValue::Long(value),
            Str(value) => SltSqlValue::Str(value),
            F32(value) => SltSqlValue::Flt(value.into_inner()),
            F64(value) => SltSqlValue::Dbl(value.into_inner()),
            Decimal(value) => SltSqlValue::Decimal(value),
            OptBool(value) => SltSqlValue::OptBool(value),
            OptI32(value) => SltSqlValue::OptInt(value),
            OptI64(value) => SltSqlValue::OptLong(value),
            OptStr(value) => SltSqlValue::OptStr(value),
            OptF32(value) => SltSqlValue::OptFlt(value.map(|value| value.into_inner())),
            OptF64(value) => SltSqlValue::OptDbl(value.map(|value| value.into_inner())),
            OptDecimal(value) => SltSqlValue::OptDecimal(value),
...
impl ToSqlRow for SqlTuple {
    fn to_row(&self) -> SqlRow {
        let mut result = SqlRow::new();
        for i in 0 .. self.len() {
            let element = &self[i];
            result.push(element.clone().into());
        }
        result
    }
}
